...
train_data.p <- train_data[,.(Survived,Pclass,Sex,Age,SibSp,Parch,Fare,Embarked,Cabin_kind,Person,Family_size)]
...
train_data$pred <- ifelse(predict(prediction_model_glm,train_data,type = 'response')>0.60,1,0)
...
test_data[,Survived:=ifelse(predict(prediction_model_glm,test_data,type = 'response')>0.605,1,0)]
